...
function NextBuys() {
  ...
  const getUserData = async () => {
    ...
    if (isSignedIn === "true") {
      ...
      axios
        .get(`${process.env.REACT_APP_BASE_URL}UserNextBuy.aspx?phone=${phone}`)
        .then(async (res) => {
  ...
  const removeFromFav = async (id) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}DeleteNextBuy.aspx?phone=${phone}&id=${id}`
      )
      .then((res) => {
        getUserData();
      })
  ...
  return (
    <Layout loading={loading} title="لیست خرید بعدی">
      {/* <!-- Breadcumbs start --> */}
      <div className="e-breadcumb-wrap text-center">
        <h2 className="e-breadcumb-title">لیست خرید بعدی</h2>
        <ul className="e-breadcumb-kist">
          <li>
            <NavLink to="/">خانه </NavLink>
          </li>
          <li>
            <NavLink to="/nextbuys">لیست خرید بعدی</NavLink>
          </li>
        </ul>
      </div>
      ...
      {isLoggedIn ? (
        wishlistItems.length > 0 ? (
          ...
          <div className="d-flex flex-column justify-content-center align-items-center">
            <div className="h3">لیست خرید بعدی شما خالیست!</div>
            <NavLink
