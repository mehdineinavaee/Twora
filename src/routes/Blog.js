...
function Blog({ match }) {
  ...
  useEffect(() => {
    console.warn(id);
    axios
      .get(`${process.env.REACT_APP_BASE_URL}Blog.aspx`)
      .then((res) => {
    ...
    <Layout loading={loading} title={(data && data.title) || ""}>
      ...
      <div className="e-breadcumb-wrap text-center">
        ...
        <ul className="e-breadcumb-kist">
          ...
          <li>
            { page == 1 ?
            <NavLink to="/blog_category/1">دسته بندی مجلات آموزشی</NavLink>
            :
            <NavLink to="/blog_category/2">دسته بندی ویدیوهای آموزشی</NavLink>
            }
          </li>
          <li>
            <NavLink to="/blog/1">
